...
from ..utils.summary import ModelException, messages_summary, send_summary
...
async def handle_summary(
    ...
    try:
        ...
        if content_value:
            logger.debug(f"最终收集到内容过滤: '{content_value}'", "总结")
        ...
        try:
            ...
            if target_user_ids:
                ...
                logger.debug(f"将对用户 {target_user_names} 进行过滤总结", "总结")
        ...
        try:
            ...
            # 类型为“Annotated”的对象不可调用PylancereportCallIssue
        ...
        if success:
            ...
            try:
                ...
                logger.error(f"记录插件调用统计失败: {stat_e}", "总结", e=stat_e)
            logger.debug(
